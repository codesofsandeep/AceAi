...
// ✅ Dynamic axios instance for dev + prod
...
const ReviewResume = () => {
  ...
  // Analyze Resume
  const handleReview = async () => {
    if (!file) return toast.error("Please upload a resume to continue");
    ...
    try {
      ...
      const res = await api.post("/ai/review-resume", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!res.data?.success) throw new Error(res.data?.message || "Analysis failed");
      ...
      console.error(error);
      toast.error(error.response?.data?.message || error.message || "Resume review failed ❌");
    } finally {
      setLoading(false);
    }
  };
        ...
        <section className="bg-[#111418]/80 border border-white/10 rounded-2xl p-8 space-y-8">
          {!reviewed ? (
            ...
            <>
              ...
              {/* STRENGTHS */}
              <div className="space-y-2">
                <h3 className="text-sm font-medium text-slate-300 mb-2">Key Strengths</h3>
                <div className="max-h-56 overflow-y-auto pr-4 bg-[#0b0e13] border border-white/10 rounded-lg p-3">
                  <ul className="space-y-2 text-sm text-slate-400 list-disc list-inside">
                    <li>Relevant technical and role-specific keywords</li>
                    <li>Clear role descriptions and project summaries</li>
